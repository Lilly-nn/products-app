...
import { setVisible } from '@/context/features/visible/visibleSlice';

export default function MainSlide() {
  const { categoriesVisible } = useTypedSelector((state) => state.visible);
  const dispatch = useDispatch();
  return (
    <>
      <div className='home__nav'>
        <div className='flex items-center'>
          <button onClick={() => dispatch(setVisible())} className='btn'>
      ...
      <section className='home__main-slide'>
        <div className='flex justify-between gap-x-6 mb-[30px]'>
          <AsideCategories />
          <div
            className='home__main-banner flex items-center'
            style={!categoriesVisible ? { width: '100%' } : {}}>
            <div className='ml-12'>
